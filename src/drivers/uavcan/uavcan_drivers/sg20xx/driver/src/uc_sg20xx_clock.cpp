...
# if UAVCAN_SG20XX_NUTTX
    ...
    volatile uavcan::uint32_t cnt = can_timer->ops->ioctl(can_timer, SG2002_Timer_Get_Current_CountVal, NULL);

    if (can_timer->ops->ioctl(can_timer, SG2002_Timer_Check_IntStatus, 0)) {
        cnt = can_timer->ops->ioctl(can_timer, SG2002_Timer_Get_Current_CountVal, NULL);
...
static void updateRatePID(uavcan::UtcDuration adjustment) {
    ...
    if (dt > 10) {
        ...
        utc_rel_rate_error_integral = uavcan::max(utc_rel_rate_error_integral, -utc_sync_params.max_rate_correction_ppm);
        utc_rel_rate_error_integral = uavcan::min(utc_rel_rate_error_integral, utc_sync_params.max_rate_correction_ppm);
    }
